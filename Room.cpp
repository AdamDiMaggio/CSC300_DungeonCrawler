...
Room::Room(string name, string resident)
{
    this->RoomName = name;
...
Room* Room::addRoom(int direction, string RoomName, string resident)   //where are we adding a room and door to
{
    if(direction == 12)  //north door
    {
        ...
        Room* room = new Room(RoomName, resident);  
        this->NorthRoom = room;
    ...
    else if(direction == 3)  //east door
    {
        ...
        Room* room = new Room(RoomName, resident);   //set room name in dungeon
        this->EastRoom = room;
